...
/// Folder Aggregate - represents a folder in the system
...
impl AggregateRoot for Folder {
    ...
    fn apply(&mut self, event: &FileSystemEvent) {
        match event {
            ...
            _ => {} // Ignore file events
        }
        self.version += 1;
    }
}

impl Securable for Folder {
...
impl SoftDeletable for Folder {
    ...
    fn mark_as_deleted(&mut self, by: Uuid) {
        let event = FileSystemEvent::FolderDeleted(FolderDeletedEvent {
            folder_id: self.id,
            deleted_by: by,
            occurred_at: Utc::now(),
    ...
    fn restore(&mut self) {
        let event = FileSystemEvent::FolderRestored(FolderRestoredEvent {
            folder_id: self.id,
...
impl Folder {
    ...
    pub fn create(
        ...
        let event = FileSystemEvent::FolderCreated(FolderCreatedEvent {
            folder_id: id,
            name: name.clone(),
            path: path.clone(),
            parent_id,
        ...
        folder.apply(&event);
        folder.uncommitted_events.push(event);
    ...
    pub fn move_to(&mut self, new_parent_id: Option<Uuid>, new_path: String, moved_by: Uuid) {
        let event = FileSystemEvent::FolderMoved(FolderMovedEvent {
            folder_id: self.id,
    ...
    pub fn rename(&mut self, new_name: String, new_path: String, renamed_by: Uuid) {
        ...
        let event = FileSystemEvent::FolderRenamed(FolderRenamedEvent {
            folder_id: self.id,
            old_name,
